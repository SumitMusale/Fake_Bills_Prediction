...
def model_training():
    ...
    image = Image.open('Fake_bill.jpeg')
...
simply from certain dimensions of the banknote and the elements that compose it.''')
    ...
    # data visualization
    # you can add html report in app (autoviz save html report)
    ...
    if chart_select == "Scatterplot":
        try:
            HtmlFile = open("AutoViz_Plots/is_genuine/pair_scatters.html", 'r', encoding='utf-8')
    ...
    if chart_select == "Distplot":
        try:
            HtmlFile = open("AutoViz_Plots/is_genuine/distplots_nums.html", 'r', encoding='utf-8')
            source_code = HtmlFile.read() 
            # print(source_code)
    ...
    if chart_select == "Heatmap":
        try:
            HtmlFile = open("AutoViz_Plots/is_genuine/heatmaps.html", 'r', encoding='utf-8')
            source_code = HtmlFile.read() 
            # print(source_code)
